...
def evalutate(val_loader, model, criterion, epoch, args, logger):
    ...
    with torch.no_grad():
        for idx, (imgs, labels) in enumerate(val_loader):
            ...
            if idx % args.print_fq == 0 or idx + 1 == len(val_loader):
                msg = 'Epoch: [{0}][{1}/{2}]\t' \
                      ...
                      'ICC {icc.val:.5f} ({icc.avg:.5f})\t' .format (
                    epoch, idx, len (val_loader), batch_time=batch_time,
                    speed=bs / batch_time.val,
                    data_time=data_time, loss=loss, mae=mae, acc=acc, icc=icc)
                logger.info (msg)
        ...
        return loss.avg, acc.avg, mae.avg, icc.avg, pl.sum, pm.sum, pred, label
